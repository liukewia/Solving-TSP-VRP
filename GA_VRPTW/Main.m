...
%% 初始化种群
Chrom=InitPop(NIND,CityNum,Distance,TravelTime,Demand,TimeWindow,Travelcon,Capacity);

%% 迭代
gen=1;
while gen <= MAXGEN
    %% 计算适应度
    [ttlDistance,FitnV]=Fitness(Chrom,Distance,TravelTime,Demand,TimeWindow,Travelcon,Capacity);  %计算路径长度
    ...
    %% 逆转操作
    SelCh=Reverse(SelCh,Distance,TravelTime,Demand,TimeWindow,Travelcon,Capacity);
    %% 亲代重插入子代
