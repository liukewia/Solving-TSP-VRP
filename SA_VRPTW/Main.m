...
for j=2:CityNum+1
	k=k+1;   %对VRP路径下一点进行赋值
	if DisTraveled+Distance(S1(k-1),TSProute(j))+Distance(TSProute(j),1) > Travelcon || delivery+Demand(TSProute(j)) > Capacity || CurrentTime+TravelTime(S1(k-1),TSProute(j))>TimeWindow(TSProute(j),2)
        S1(k)=1; %经过配送中心
        ...
        CurrentTime=max(CurrentTime+TravelTime(S1(k-1),TSProute(j)),TimeWindow(TSProute(j),1));%若到达时间早于早时间窗，等待至早时间窗
...
while T0 > Tend
    ...
    for k = 1:L
        ...
        [S1,ttlDis] = Metropolis(S1,S2,Distance,TravelTime,Demand,TimeWindow,Travelcon,Capacity,T0);  % Metropolis 抽样算法
